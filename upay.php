<?php 
/*
Plugin Name: wc-upay-manual-payment-gateway
Plugin URI:  https://hasib.w3spaces.com/
...
Version:     2.0
Author:      hasibmhmd
Author URI:  https://hasib.w3spaces.com/
License:     GPL2
...
if( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ){
	 ...
	 */
	add_filter('woocommerce_payment_gateways', 'hasib_upay_payment_gateways');
	function hasib_upay_payment_gateways( $gateways ){
		$gateways[] = 'hasib_upay';
		return $gateways;
	}
	 ...
	 * upay gateway init
	 */
	add_action('plugins_loaded', 'hasib_upay_plugin_activation');
	function hasib_upay_plugin_activation(){
		
		class hasib_upay extends WC_Payment_Gateway {
			...
			public function __construct(){
				...
				$this->hasib_upay_options_fields();
				$this->init_settings();
	            ...
	            add_filter( 'woocommerce_thankyou_order_received_text', array( $this, 'hasib_upay_thankyou_page' ) );
	            add_action( 'woocommerce_email_before_order_table', array( $this, 'hasib_upay_number_instructions' ), 10, 3 );
			}


			public function hasib_upay_options_fields(){
	        ...
	        public function hasib_upay_thankyou_page() {
	        ...
	        public function hasib_upay_number_instructions( $order, $sent_to_admin, $plain_text = false ) {
			    if( $order->payment_method != $this->id )
			        return;        	
	 ...
	 */
	add_filter( "plugin_action_links_". plugin_basename(__FILE__), 'hasib_upay_settings_link' );
	function hasib_upay_settings_link( $links ) {
		
		$settings_links = array();
		$settings_links[] ='<a href="https://www.facebook.com/hasibvaai/" target="_blank">' . esc_html__( 'Follow US', 'stb' ) . '</a>';
		$settings_links[] ='<a href="' . admin_url( 'admin.php?page=wc-settings&tab=checkout&section=hasib_upay' ) . '">' . esc_html__( 'Settings', 'stb' ) . '</a>';
	 ...
	 */
	$upay_charge = get_option( 'woocommerce_hasib_upay_settings' );
	if( $upay_charge['upay_charge'] == 'yes' ){

		add_action( 'wp_enqueue_scripts', 'hasib_upay_script' );
		function hasib_upay_script(){
			wp_enqueue_script( 'stb-script', plugins_url( 'js/scripts.js', __FILE__ ), array('jquery'), '1.0', true );
		}

		add_action( 'woocommerce_cart_calculate_fees', 'hasib_upay_charge' );
		function hasib_upay_charge(){
		    ...
		    if( $current_gateway!='' ){
				...
				if ( $current_gateway_id =='hasib_upay' ) {
					$percentage = 0.02;
	 ...
	 */
	add_action( 'woocommerce_checkout_process', 'hasib_upay_payment_process' );
	function hasib_upay_payment_process(){

	    if($_POST['payment_method'] != 'hasib_upay')
	        return;
	 ...
	 */
	add_action( 'woocommerce_checkout_update_order_meta', 'hasib_upay_additional_fields_update' );
	function hasib_upay_additional_fields_update( $order_id ){
	 ...
	 */
	add_action('woocommerce_admin_order_data_after_billing_address', 'hasib_upay_admin_order_data' );
	function hasib_upay_admin_order_data( $order ){
	    
	    if( $order->payment_method != 'hasib_upay' )
	        return;
	 ...
	 */
	add_action('woocommerce_order_details_after_customer_details', 'hasib_upay_additional_info_order_review_fields' );
	function hasib_upay_additional_info_order_review_fields( $order ){
	 ...
	 */
	add_filter( 'manage_edit-shop_order_columns', 'hasib_upay_admin_new_column' );
	function hasib_upay_admin_new_column($columns){
	 ...
	 */
	add_action( 'manage_shop_order_posts_custom_column', 'hasib_upay_admin_column_value', 2 );
	function hasib_upay_admin_column_value($column){
	 ...
	 * Admin Notice
	 */
	add_action( 'admin_notices', 'hasib_upay_admin_notice__error' );
	function hasib_upay_admin_notice__error() {
	    ?>
	 ...
	 * Deactivate Plugin
	 */
	add_action( 'admin_init', 'hasib_upay_deactivate' );
	function hasib_upay_deactivate() {
